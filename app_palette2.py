...
# Color Palette 2 Executive Theme UI/UX Design System
st.markdown("""
<style>
    ...
    .stApp {
        ...
        background-image: 
            radial-gradient(at 50% 0%, #0B3854 0%, transparent 75%),
            radial-gradient(at 0% 100%, rgba(39, 211, 140, 0.15) 0px, transparent 50%),
            radial-gradient(at 100% 100%, rgba(124, 191, 241, 0.15) 0px, transparent 50%);
    ...
    .command-hero-header {
        background: linear-gradient(135deg, #001C32 0%, #0B3854 50%, #001C32 100%);
        padding: 2.2rem 3rem;
        border-radius: 24px;
        color: #EAFBEE;
        margin-bottom: 2rem;
        box-shadow: 0 16px 40px rgba(0, 28, 50, 0.8), 0 0 30px rgba(39, 211, 140, 0.2);
        border: 1.5px solid rgba(39, 211, 140, 0.4);
    }
    .command-badge {
        display: inline-block;
        background: rgba(39, 211, 140, 0.2);
        color: #71D98C;
        border: 1px solid #27D38C;
        font-weight: 800;
        font-size: 0.8rem;
        padding: 5px 14px;
        border-radius: 20px;
    ...
    button[data-testid="stBaseButton-secondary"] {
        ...
        border: 1.5px solid rgba(39, 211, 140, 0.35) !important;
        border-radius: 18px !important;
        box-shadow: 0 6px 18px rgba(0, 28, 50, 0.5), 0 0 15px rgba(39, 211, 140, 0.15) !important;
    ...
    button[data-testid="stPopoverButton"]:hover {
        ...
        box-shadow: 0 10px 24px rgba(0, 28, 50, 0.7), 0 0 25px rgba(39, 211, 140, 0.25) !important;
    ...
    div[data-testid="stPopoverBody"] {
        background: rgba(8, 42, 70, 0.96) !important;
        backdrop-filter: blur(16px) !important;
        border: 1.5px solid rgba(39, 211, 140, 0.4) !important;
        border-radius: 20px !important;
        box-shadow: 0 16px 40px rgba(0, 28, 50, 0.8), 0 0 30px rgba(39, 211, 140, 0.2) !important;
        padding: 1.4rem !important;
    }

    /* High-Contrast Time Period Chip Badge inside Popover Container */
    .period-chip {
        background: rgba(39, 211, 140, 0.2) !important;
        border: 1.5px solid #27D38C !important;
        ...
        box-shadow: 0 4px 12px rgba(0, 28, 50, 0.5), 0 0 15px rgba(39, 211, 140, 0.25) !important;
    }
    ...
    div[data-testid="stSelectbox"] > div {
        ...
        border-radius: 14px !important;
        box-shadow: 0 4px 12px rgba(0, 28, 50, 0.05) !important;
        transition: all 0.22s ease !important;
    }
    div[data-testid="stSelectbox"] > div:hover {
        border-color: #27D38C !important;
        box-shadow: 0 6px 18px rgba(39, 211, 140, 0.25) !important;
    }
    ...
    /* KPI Cards Styling for Palette 2 */
    .metric-card-exec {
        background: #102F4F !important;
        border-radius: 22px;
        padding: 1.4rem 1.1rem;
        text-align: center;
        box-shadow: 0 10px 28px rgba(0, 28, 50, 0.6), 0 0 20px rgba(39, 211, 140, 0.15);
        transition: all 0.28s cubic-bezier(0.4, 0, 0.2, 1);
        position: relative;
        overflow: hidden;
        border: 1.5px solid rgba(39, 211, 140, 0.35);
    }
    .metric-card-exec:hover {
        background: #1B3561 !important;
        transform: translateY(-6px);
        box-shadow: 0 16px 36px rgba(0, 28, 50, 0.8), 0 0 30px rgba(39, 211, 140, 0.3);
        border-color: #27D38C;
    }
    ...
    /* 🟢 Headcount (Emerald Green) */
    .kpi-emerald {
        background: #102F4F !important;
        border-color: rgba(39, 211, 140, 0.4);
    }
    .kpi-emerald::before { background: linear-gradient(90deg, #27D38C, #71D98C); }
    .metric-val-emerald { font-size: 2.3rem; font-weight: 800; color: #27D38C; line-height: 1.1; }
    .badge-bg-emerald { background: rgba(39, 211, 140, 0.2); color: #EAFBEE; border: 1px solid #27D38C; }

    /* 🟧 Active JDA / QtQ % (Mint Highlight) */
    .kpi-orange {
        background: #102F4F !important;
        border-color: rgba(113, 217, 140, 0.4);
    }
    .kpi-orange::before { background: linear-gradient(90deg, #71D98C, #7CBFF1); }
    .metric-val-orange { font-size: 2.3rem; font-weight: 800; color: #71D98C; line-height: 1.1; }
    .badge-bg-orange { background: rgba(113, 217, 140, 0.2); color: #EAFBEE; border: 1px solid #71D98C; }

    /* 🟦 Active ME / YoY % (Teal Blue) */
    .kpi-blue {
        background: #102F4F !important;
        border-color: rgba(124, 191, 241, 0.4);
    }
    .kpi-blue::before { background: linear-gradient(90deg, #7CBFF1, #27D38C); }
    .metric-val-blue { font-size: 2.3rem; font-weight: 800; color: #7CBFF1; line-height: 1.1; }
    .badge-bg-blue { background: rgba(124, 191, 241, 0.2); color: #EAFBEE; border: 1px solid #7CBFF1; }

    /* 🔴 Active TME / Exited (Emerald Accent) */
    .kpi-rose {
        background: #102F4F !important;
        border-color: rgba(39, 211, 140, 0.4);
    }
    .kpi-rose::before { background: linear-gradient(90deg, #27D38C, #7CBFF1); }
    .metric-val-rose { font-size: 2.3rem; font-weight: 800; color: #27D38C; line-height: 1.1; }
    .badge-bg-rose { background: rgba(39, 211, 140, 0.2); color: #EAFBEE; border: 1px solid #27D38C; }
    ...
    .kpi-cyan::before { background: linear-gradient(90deg, #7CBFF1, #71D98C); }
    .metric-val-cyan { font-size: 2.3rem; font-weight: 800; color: #7CBFF1; line-height: 1.1; }
    .badge-bg-cyan { background: rgba(124, 191, 241, 0.2); color: #EAFBEE; border: 1px solid #7CBFF1; }
    ...
    .custom-table-card {
        ...
        padding: 1.6rem;
        border: 1.5px solid rgba(39, 211, 140, 0.35);
        box-shadow: 0 12px 36px rgba(0, 28, 50, 0.6), 0 0 25px rgba(39, 211, 140, 0.2);
        margin-bottom: 2rem;
        overflow-x: auto;
    }
    .custom-table {
    ...
    .custom-table th {
        background: linear-gradient(135deg, #001C32 0%, #0B3854 100%);
        color: #EAFBEE;
        font-weight: 800;
        padding: 16px 20px;
        text-align: center;
        border-bottom: 3px solid #27D38C;
        font-size: 0.9rem;
        text-transform: uppercase;
    ...
    .custom-table td {
        padding: 14px 20px;
        text-align: center;
        border-bottom: 1px solid rgba(39, 211, 140, 0.2);
        color: #EAFBEE !important;
        font-weight: 600;
    }
    .custom-table tr:nth-child(even) td {
        background-color: rgba(8, 42, 70, 0.6);
    }
    .custom-table tr:hover td {
        background-color: #1B3561 !important;
    }
    .custom-table tr.total-row td {
        background: linear-gradient(135deg, #27D38C 0%, #71D98C 50%, #7CBFF1 100%) !important;
        color: #001C32 !important;
        font-weight: 800 !important;
    }
    ...
    .export-footer {
        ...
        backdrop-filter: blur(12px);
        border: 1px solid #27D38C;
        border-top: 4px solid #71D98C;
        border-radius: 20px;
        padding: 1.8rem 2.4rem;
        margin-top: 2.5rem;
        margin-bottom: 2rem;
        box-shadow: 0 12px 30px rgba(0, 28, 50, 0.6), 0 0 25px rgba(39, 211, 140, 0.25);
    }
    .export-title {
        font-size: 1.25rem;
        font-weight: 800;
    ...
    .stTabs [data-baseweb="tab-list"] {
        ...
        box-shadow: 0 8px 26px rgba(0, 28, 50, 0.6), 0 0 20px rgba(39, 211, 140, 0.2);
        border: 1.5px solid rgba(39, 211, 140, 0.35);
    ...
    .stTabs [data-baseweb="tab"]:hover {
        background-color: rgba(39, 211, 140, 0.2) !important;
        color: #7CBFF1 !important;
    }
    .stTabs [aria-selected="true"] {
        color: #001C32 !important;
        background: linear-gradient(135deg, #27D38C 0%, #71D98C 50%, #7CBFF1 100%) !important;
        border-radius: 16px !important;
        box-shadow: 0 8px 22px rgba(0, 28, 50, 0.6), 0 0 20px rgba(39, 211, 140, 0.4) !important;
        border-bottom: none !important;
    }
    ...
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] {
        ...
        box-shadow: inset 0 2px 4px rgba(0, 28, 50, 0.4), 0 0 15px rgba(39, 211, 140, 0.15) !important;
    }
    ...
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] label:has(input:checked) {
        ...
        font-weight: 800 !important;
        border-radius: 12px !important;
        box-shadow: 0 4px 14px rgba(0, 28, 50, 0.6), 0 0 18px rgba(39, 211, 140, 0.4) !important;
    }
</style>
...
<script>
    (function() {
        try {
            ...
            var styleId = 'custom-smooth-scroll-style-p2';
            ...
            if (!targetDoc._hasExpanderPersistListenerP2) {
                targetDoc._hasExpanderPersistListenerP2 = true;
...
def render_download_section(section_id="default"):
    ...
    <div class="export-footer">
        ...
        <div style="font-size: 0.92rem; color: #7CBFF1; margin-bottom: 0.9rem; font-weight:600;">
    ...
    if df_filtered.empty:
        ...
        if s_mode == "📊 Pan India / Branch Summary":
            ...
            for _, r in br_hc_df.iterrows():
                html_b.append('<tr>')
                html_b.append(f'<td style="font-weight:700; text-align:left;">{r["branch"]}</td>')
                html_b.append(f'<td style="color:#71D98C; font-weight:800;">{r["Active_Count"]}</td>')
                html_b.append(f'<td style="color:#27D38C; font-weight:700;">{r["Exited_Count"]}</td>')
                html_b.append(f'<td style="color:#7CBFF1; font-weight:800;">{r["Active_Pct"]}</td>')
                html_b.append('</tr>')
            ...
            for _, r in team_hc_df.iterrows():
                html_t.append('<tr>')
                html_t.append(f'<td style="font-weight:700; text-align:left;">{r["team_type"]}</td>')
                html_t.append(f'<td style="color:#71D98C; font-weight:800;">{r["Active_Count"]}</td>')
                html_t.append(f'<td style="color:#27D38C; font-weight:700;">{r["Exited_Count"]}</td>')
                html_t.append(f'<td style="color:#7CBFF1; font-weight:800;">{r["Active_Pct"]}</td>')
                html_t.append('</tr>')
    ...
    if df_filtered.empty:
        ...
        if v_mode == "⏳ Overall Tenure Buckets":
            ...
            for _, r in tb_df.iterrows():
                html_tb.append('<tr>')
                html_tb.append(f'<td style="font-weight:700; text-align:left; color:#7CBFF1;">{r["tenure_bucket"]}</td>')
                html_tb.append(f'<td style="color:#71D98C; font-weight:800;">{int(r["Active_Count"])}</td>')
                html_tb.append(f'<td style="color:#27D38C; font-weight:700;">{int(r["Exited_Count"])}</td>')
                html_tb.append(f'<td style="color:#7CBFF1; font-weight:800;">{r["Active_Pct"]}</td>')
                html_tb.append('</tr>')
            ...
            for _, r in b_pivot.iterrows():
                html_bp.append('<tr>')
                html_bp.append(f'<td style="font-weight:700; text-align:left;">{r["branch"]}</td>')
                for bo in bucket_order:
                    html_bp.append(f'<td>{r[bo]}</td>')
                html_bp.append(f'<td style="font-weight:800; color:#7CBFF1;">{r["Headcount"]}</td>')
                html_bp.append('</tr>')
            ...
            for _, r in t_pivot.iterrows():
                html_tp.append('<tr>')
                html_tp.append(f'<td style="font-weight:700; text-align:left;">{r["team_type"]}</td>')
                for bo in bucket_order:
                    html_tp.append(f'<td>{r[bo]}</td>')
                html_tp.append(f'<td style="font-weight:800; color:#7CBFF1;">{r["Headcount"]}</td>')
                html_tp.append('</tr>')
    ...
    if d_mode == "🔍 Searchable Employee List":
        ...
        if display_df.empty:
            ...
            for _, r in display_df.head(100).iterrows():
                ...
                html5.append('<tr>')
                html5.append(f'<td style="font-weight:700; color:#7CBFF1;">{r["emp_code"]}</td>')
                html5.append(f'<td style="text-align:left; font-weight:600;">{r["emp_name"]}</td>')
                html5.append(f'<td>{r["branch"]}</td>')
                ...
                html5.append(f'<td style="font-weight:800; color:#27D38C;">{r["tenure_months"]} M</td>')
                html5.append(f'<td>{r["tenure_years"]} Yrs</td>')
                html5.append(f'<td>{r["tenure_days"]} Days</td>')
                html5.append(f'<td style="font-size:0.82rem; color:#7F9DB5;">{r["designation"]}</td>')
                html5.append('</tr>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_b_df.iterrows():
                ...
                html3.append(f'<td style="text-align:left; font-weight:700;">{r["Branch / City"]}</td>')
                html3.append(f'<td style="color:#71D98C; font-weight:800;">{r["Headcount"]}</td>')
                html3.append(f'<td style="color:#27D38C; font-weight:700;">{r["Exited in Period"]}</td>')
                html3.append(f'<td style="font-weight:800; color:#7CBFF1;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_t_df.iterrows():
                ...
                html4.append(f'<td style="text-align:left; font-weight:700;">{r["Team Type"]}</td>')
                html4.append(f'<td style="color:#71D98C; font-weight:800;">{r["Headcount"]}</td>')
                html4.append(f'<td style="color:#27D38C; font-weight:700;">{r["Exited in Period"]}</td>')
                html4.append(f'<td style="font-weight:800; color:#7CBFF1;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if exited_df.empty:
            ...
            for _, r in exited_df.iterrows():
                ...
                html_ex.append('<tr>')
                html_ex.append(f'<td style="font-weight:700; color:#27D38C;">{r["emp_code"]}</td>')
                html_ex.append(f'<td style="text-align:left; font-weight:600;">{r["emp_name"]}</td>')
                ...
                html_ex.append(f'<td style="color:#27D38C; font-weight:800;">{dol_str}</td>')
                html_ex.append(f'<td style="font-weight:800; color:#7CBFF1;">{r["tenure_months"]} M</td>')
                html_ex.append(f'<td>{r["tenure_years"]} Yrs</td>')
                html_ex.append(f'<td style="font-size:0.82rem; color:#7F9DB5;">{r["designation"]}</td>')
                html_ex.append(f'<td style="font-size:0.82rem;">{r["email"]}</td>')
                html_ex.append(f'<td style="font-size:0.82rem;">{r["phone"]}</td>')
                html_ex.append('</tr>')
