...
# Obsidian Midnight & Electric Neon Theme (Color Palette 7)
st.markdown("""
<style>
    ...
    .stApp {
        ...
        background-image: 
            radial-gradient(at 50% 0%, #111A2E 0%, transparent 75%),
            radial-gradient(at 0% 100%, rgba(59, 130, 246, 0.18) 0px, transparent 50%),
    ...
    .command-hero-header {
        background: linear-gradient(135deg, #0B1120 0%, #172238 50%, #111A2E 100%);
        padding: 2.2rem 3rem;
        border-radius: 24px;
        color: #E2E8F0;
        margin-bottom: 2rem;
        box-shadow: 0 16px 40px rgba(11, 17, 32, 0.8), 0 0 30px rgba(59, 130, 246, 0.25);
        border: 1.5px solid #2A3B59;
    }
    .command-badge {
    ...
    button[data-testid="stBaseButton-secondary"] {
        ...
        border: 1.5px solid #2A3B59 !important;
        border-radius: 18px !important;
        box-shadow: 0 6px 18px rgba(11, 17, 32, 0.5), 0 0 15px rgba(59, 130, 246, 0.15) !important;
    ...
    button[data-testid="stPopoverButton"]:hover {
        ...
        box-shadow: 0 10px 24px rgba(11, 17, 32, 0.7), 0 0 25px rgba(59, 130, 246, 0.3) !important;
    ...
    /* Floating Popover Container Body Styling (#16243B) */
    div[data-testid="stPopoverBody"] {
        background: rgba(22, 36, 59, 0.96) !important;
        backdrop-filter: blur(16px) !important;
        border: 1.5px solid #2A3B59 !important;
        border-radius: 20px !important;
        box-shadow: 0 16px 40px rgba(11, 17, 32, 0.8), 0 0 30px rgba(59, 130, 246, 0.2) !important;
    ...
    .period-chip {
        ...
        box-shadow: 0 4px 12px rgba(11, 17, 32, 0.5), 0 0 15px rgba(59, 130, 246, 0.3) !important;
    }
    ...
    div[data-testid="stSelectbox"] > div {
        ...
        border-radius: 14px !important;
        box-shadow: 0 4px 12px rgba(11, 17, 32, 0.05) !important;
    ...
    /* KPI Cards Styling (#1C2942 Card Background, #22314D Hover) */
    .metric-card-exec {
        background: #1C2942 !important;
        border-radius: 22px;
        padding: 1.4rem 1.1rem;
        text-align: center;
        box-shadow: 0 10px 28px rgba(11, 17, 32, 0.5), 0 0 20px rgba(59, 130, 246, 0.15);
    ...
    .metric-card-exec:hover {
        ...
        box-shadow: 0 16px 36px rgba(11, 17, 32, 0.7), 0 0 30px rgba(77, 163, 255, 0.3);
        border-color: #4DA3FF;
    }
    ...
    /* 🟢 Headcount (Mint Highlight #34D399) */
    .kpi-emerald {
        background: #1C2942 !important;
        border-color: rgba(52, 211, 153, 0.4);
    }
    .kpi-emerald::before { background: linear-gradient(90deg, #34D399, #22D3EE); }
    .metric-val-emerald { font-size: 2.3rem; font-weight: 800; color: #34D399; line-height: 1.1; }
    .badge-bg-emerald { background: rgba(52, 211, 153, 0.2); color: #E2E8F0; border: 1px solid #34D399; }

    /* 🟧 Active JDA / QtQ % (Cyan #22D3EE) */
    .kpi-orange {
        background: #1C2942 !important;
        border-color: rgba(34, 211, 238, 0.4);
    }
    .kpi-orange::before { background: linear-gradient(90deg, #22D3EE, #4DA3FF); }
    .metric-val-orange { font-size: 2.3rem; font-weight: 800; color: #22D3EE; line-height: 1.1; }
    .badge-bg-orange { background: rgba(34, 211, 238, 0.2); color: #E2E8F0; border: 1px solid #22D3EE; }

    /* 🟦 Active ME / YoY % (Violet #8B5CF6) */
    .kpi-blue {
        background: #1C2942 !important;
        border-color: rgba(139, 92, 246, 0.4);
    }
    .kpi-blue::before { background: linear-gradient(90deg, #8B5CF6, #A78BFA); }
    .metric-val-blue { font-size: 2.3rem; font-weight: 800; color: #8B5CF6; line-height: 1.1; }
    .badge-bg-blue { background: rgba(139, 92, 246, 0.2); color: #E2E8F0; border: 1px solid #8B5CF6; }

    /* 🔴 Active TME / Exited (Alert Rose #FB7185) */
    .kpi-rose {
        background: #1C2942 !important;
        border-color: rgba(251, 113, 133, 0.4);
    }
    .kpi-rose::before { background: linear-gradient(90deg, #FB7185, #FBBF24); }
    .metric-val-rose { font-size: 2.3rem; font-weight: 800; color: #FB7185; line-height: 1.1; }
    .badge-bg-rose { background: rgba(251, 113, 133, 0.2); color: #E2E8F0; border: 1px solid #FB7185; }

    /* 🌊 MoM % (Electric Blue #4DA3FF) */
    .kpi-cyan {
        background: #1C2942 !important;
        border-color: rgba(77, 163, 255, 0.4);
    }
    .kpi-cyan::before { background: linear-gradient(90deg, #3B82F6, #4DA3FF); }
    .metric-val-cyan { font-size: 2.3rem; font-weight: 800; color: #4DA3FF; line-height: 1.1; }
    .badge-bg-cyan { background: rgba(77, 163, 255, 0.2); color: #E2E8F0; border: 1px solid #4DA3FF; }
    ...
    /* Executive Tables Card Container (#16243B Table Background) */
    .custom-table-card {
        ...
        box-shadow: 0 16px 40px rgba(11, 17, 32, 0.8), 0 0 30px rgba(59, 130, 246, 0.3);
        margin-bottom: 2rem;
        overflow-x: auto;
    }
    .custom-table {
    ...
    .custom-table th {
        background: linear-gradient(135deg, #172238 0%, #22314D 100%) !important;
    ...
    .custom-table tr:nth-child(odd) td {
        background-color: #16243B !important; /* Normal row */
    }
    .custom-table tr:nth-child(even) td {
        background-color: #1B2B46 !important; /* Alternate row */
    }
    .custom-table tr:hover td {
        background-color: #263B5E !important; /* Hover row */
        color: #FFFFFF !important;
    }
    .custom-table tr.total-row td {
        background: linear-gradient(135deg, #3B82F6 0%, #4DA3FF 100%) !important;
        color: #FFFFFF !important;
        font-weight: 800 !important;
    ...
    /* Export & Download Footer Banner (#1C2942) */
    .export-footer {
        background: #1C2942 !important;
        ...
        margin-top: 2.5rem;
        margin-bottom: 2rem;
        box-shadow: 0 12px 30px rgba(11, 17, 32, 0.6), 0 0 25px rgba(59, 130, 246, 0.25);
    }
    .export-title {
        font-size: 1.25rem;
        font-weight: 800;
    ...
    /* Main Tab Navigation Bar (#172238 Track) */
    .stTabs [data-baseweb="tab-list"] {
        ...
        box-shadow: 0 8px 26px rgba(11, 17, 32, 0.6), 0 0 20px rgba(59, 130, 246, 0.2);
        border: 1.5px solid #2A3B59;
    ...
    .stTabs div[data-baseweb="tab"][aria-selected="true"] span {
        ...
        background: linear-gradient(135deg, #3B82F6 0%, #4DA3FF 100%) !important;
        border-radius: 16px !important;
        box-shadow: 0 8px 22px rgba(11, 17, 32, 0.6), 0 0 20px rgba(59, 130, 246, 0.4) !important;
        border-bottom: none !important;
        font-weight: 800 !important;
    }
    ...
    /* Integrated Parent-Child Sub-View Segmented Control Bar (#172238) */
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] {
        ...
        box-shadow: inset 0 2px 4px rgba(11, 17, 32, 0.4), 0 0 15px rgba(59, 130, 246, 0.15) !important;
    }
    ...
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] label:has(input:checked) {
        ...
        font-weight: 800 !important;
        border-radius: 14px !important;
        box-shadow: 0 6px 18px rgba(11, 17, 32, 0.6), 0 0 18px rgba(59, 130, 246, 0.4) !important;
    }
</style>
...
<script>
    (function() {
        try {
            ...
            var styleId = 'custom-smooth-scroll-style-p7';
            ...
            if (!targetDoc._hasExpanderPersistListenerP7) {
                targetDoc._hasExpanderPersistListenerP7 = true;
...
def render_download_section(section_id="default"):
    ...
    <div class="export-footer">
        ...
        <div style="font-size: 0.92rem; color: #4DA3FF; margin-bottom: 0.9rem; font-weight:600;">
    ...
    if df_filtered.empty:
        ...
        if s_mode == "📊 Pan India / Branch Summary":
            ...
            for _, r in br_hc_df.iterrows():
                html_b.append('<tr>')
                html_b.append(f'<td style="font-weight:700; text-align:left; color:#FBBF24;">{r["branch"]}</td>')
                html_b.append(f'<td style="color:#34D399; font-weight:800;">{r["Active_Count"]}</td>')
                html_b.append(f'<td style="color:#FB7185; font-weight:700;">{r["Exited_Count"]}</td>')
                html_b.append(f'<td style="color:#22D3EE; font-weight:800;">{r["Active_Pct"]}</td>')
                html_b.append('</tr>')
            ...
            for _, r in team_hc_df.iterrows():
                html_t.append('<tr>')
                html_t.append(f'<td style="font-weight:700; text-align:left; color:#FBBF24;">{r["team_type"]}</td>')
                html_t.append(f'<td style="color:#34D399; font-weight:800;">{r["Active_Count"]}</td>')
                html_t.append(f'<td style="color:#FB7185; font-weight:700;">{r["Exited_Count"]}</td>')
                html_t.append(f'<td style="color:#22D3EE; font-weight:800;">{r["Active_Pct"]}</td>')
                html_t.append('</tr>')
    ...
    if df_filtered.empty:
        ...
        if v_mode == "⏳ Overall Tenure Buckets":
            ...
            for _, r in tb_df.iterrows():
                html_tb.append('<tr>')
                html_tb.append(f'<td style="font-weight:700; text-align:left; color:#FBBF24;">{r["tenure_bucket"]}</td>')
                html_tb.append(f'<td style="color:#34D399; font-weight:800;">{int(r["Active_Count"])}</td>')
                html_tb.append(f'<td style="color:#FB7185; font-weight:700;">{int(r["Exited_Count"])}</td>')
                html_tb.append(f'<td style="color:#22D3EE; font-weight:800;">{r["Active_Pct"]}</td>')
                html_tb.append('</tr>')
            ...
            for _, r in b_pivot.iterrows():
                html_bp.append('<tr>')
                html_bp.append(f'<td style="font-weight:700; text-align:left; color:#FBBF24;">{r["branch"]}</td>')
                for bo in bucket_order:
                    html_bp.append(f'<td>{r[bo]}</td>')
                html_bp.append(f'<td style="font-weight:800; color:#22D3EE;">{r["Headcount"]}</td>')
                html_bp.append('</tr>')
            ...
            for _, r in t_pivot.iterrows():
                html_tp.append('<tr>')
                html_tp.append(f'<td style="font-weight:700; text-align:left; color:#FBBF24;">{r["team_type"]}</td>')
                for bo in bucket_order:
                    html_tp.append(f'<td>{r[bo]}</td>')
                html_tp.append(f'<td style="font-weight:800; color:#22D3EE;">{r["Headcount"]}</td>')
                html_tp.append('</tr>')
    ...
    if d_mode == "🔍 Searchable Employee List":
        ...
        if display_df.empty:
            ...
            for _, r in display_df.head(100).iterrows():
                ...
                html5.append('<tr>')
                html5.append(f'<td style="font-weight:700; color:#4DA3FF;">{r["emp_code"]}</td>')
                html5.append(f'<td style="text-align:left; font-weight:700; color:#E2E8F0;">{r["emp_name"]}</td>')
                html5.append(f'<td style="color:#A78BFA;">{r["branch"]}</td>')
                ...
                html5.append(f'<td style="font-size:0.9rem; color:#A8B6CC;">{r["designation"]}</td>')
                html5.append('</tr>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_b_df.iterrows():
                ...
                html3.append(f'<td style="text-align:left; font-weight:700; color:#FBBF24;">{r["Branch / City"]}</td>')
                html3.append(f'<td style="color:#34D399; font-weight:800;">{r["Headcount"]}</td>')
                html3.append(f'<td style="color:#FB7185; font-weight:700;">{r["Exited in Period"]}</td>')
                html3.append(f'<td style="font-weight:800; color:#22D3EE;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_t_df.iterrows():
                ...
                html4.append(f'<td style="text-align:left; font-weight:700; color:#FBBF24;">{r["Team Type"]}</td>')
                html4.append(f'<td style="color:#34D399; font-weight:800;">{r["Headcount"]}</td>')
                html4.append(f'<td style="color:#FB7185; font-weight:700;">{r["Exited in Period"]}</td>')
                html4.append(f'<td style="font-weight:800; color:#22D3EE;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if exited_df.empty:
            ...
            for _, r in exited_df.iterrows():
                ...
                html_ex.append('<tr>')
                html_ex.append(f'<td style="font-weight:700; color:#FB7185;">{r["emp_code"]}</td>')
                html_ex.append(f'<td style="text-align:left; font-weight:700; color:#E2E8F0;">{r["emp_name"]}</td>')
                ...
                html_ex.append(f'<td style="color:#FB7185; font-weight:800;">{dol_str}</td>')
                html_ex.append(f'<td style="font-weight:800; color:#22D3EE;">{r["tenure_months"]} M</td>')
                html_ex.append(f'<td>{r["tenure_years"]} Yrs</td>')
                html_ex.append(f'<td style="font-size:0.9rem; color:#A8B6CC;">{r["designation"]}</td>')
