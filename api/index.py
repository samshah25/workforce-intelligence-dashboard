...
# Sample Data Generator for Vercel Serverless
def get_dataset():
...
def home():
    ...
    # Calculate KPIs
    active_df = df[df['status_as_of_obs'] == 'Active']
    total_headcount = len(active_df)
    active_jda = len(active_df[active_df['emp_type'] == 'JDA'])
    active_me = len(active_df[active_df['emp_type'] == 'ME'])
    active_tme = len(active_df[active_df['emp_type'] == 'TME'])
    
    # Render Executive Dashboard Template
    html = f"""
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            * {{ box-sizing: border-box; margin: 0; padding: 0; }}
            ...
            .kpi-card:hover {{
                ...
                box-shadow: 0 16px 36px rgba(94, 45, 145, 0.15);
            }}
            ...
            th {{
                background: linear-gradient(90deg, #5E2D91 0%, #0F172A 100%);
                color: white;
                padding: 14px;
                text-align: left;
                font-weight: 800;
    ...
    <body>
        ...
        <div class="kpi-container">
            <div class="kpi-card hc">
                <div class="kpi-val">{total_headcount}</div>
                <div class="kpi-title">TOTAL HEADCOUNT</div>
            </div>
            <div class="kpi-card jda">
                <div class="kpi-val">{active_jda}</div>
                <div class="kpi-title">ACTIVE JDA</div>
            </div>
            <div class="kpi-card me">
                <div class="kpi-val">{active_me}</div>
                <div class="kpi-title">ACTIVE ME</div>
            </div>
            <div class="kpi-card tme">
                <div class="kpi-val">{active_tme}</div>
                <div class="kpi-title">ACTIVE TME</div>
            </div>
        </div>
        ...
        <div class="table-card">
            <h2>🏢 Active Employee Roster Summary</h2>
            <table>
                <thead>
                    <tr>
                        <th>Emp Code</th>
                ...
                <tbody>
                    {''.join([f"<tr><td><b>{r['emp_code']}</b></td><td>{r['emp_name']}</td><td>{r['branch']}</td><td><b>{r['emp_type']}</b></td><td>{r['team_type']}</td><td>{r['designation']}</td><td style='color:#059669; font-weight:800;'>Active</td></tr>" for _, r in active_df.head(15).iterrows()])}
                </tbody>
            </table>
        </div>
