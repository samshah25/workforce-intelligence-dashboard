...
# Royal Slate & Indigo Light Foundation (Color Palette 5)
st.markdown("""
<style>
    ...
    .stApp {
        ...
        background-image: 
            radial-gradient(at 50% 0%, #DCE8FF 0%, transparent 75%),
            radial-gradient(at 0% 100%, #F8FAFC 0px, transparent 50%),
            radial-gradient(at 100% 100%, rgba(220, 232, 255, 0.5) 0px, transparent 50%);
    ...
    .command-hero-header {
        background: linear-gradient(135deg, #2546A8 0%, #3157C7 50%, #2546A8 100%);
        padding: 2.2rem 3rem;
        border-radius: 24px;
        color: #FFFFFF;
        margin-bottom: 2rem;
        box-shadow: 0 16px 40px rgba(49, 87, 199, 0.25);
    ...
    button[data-testid="stPopoverButton"]:hover {
        ...
        box-shadow: 0 10px 24px rgba(49, 87, 199, 0.18) !important;
    ...
    div[data-testid="stPopoverBody"] {
        ...
        box-shadow: 0 16px 40px rgba(23, 32, 51, 0.12) !important;
    ...
    .period-chip {
        ...
        box-shadow: 0 4px 12px rgba(49, 87, 199, 0.12) !important;
    }
    ...
    /* Executive Selectbox Direct Card Styling */
    ...
    /* KPI Cards Styling (#FFFFFF Card Background, #F0F5FF Hover) */
    .metric-card-exec {
        background: #FFFFFF !important;
        border-radius: 22px;
        padding: 1.4rem 1.1rem;
        text-align: center;
        box-shadow: 0 10px 28px rgba(23, 32, 51, 0.08), 0 0 20px rgba(49, 87, 199, 0.05);
    ...
    .metric-card-exec:hover {
        ...
        box-shadow: 0 16px 36px rgba(49, 87, 199, 0.18), 0 0 30px rgba(79, 142, 247, 0.15);
        border-color: #4F8EF7;
    }
    ...
    /* 🟢 Headcount (Royal Blue Highlight) */
    .kpi-emerald {
        background: #FFFFFF !important;
        border-color: #D9E1EC;
    }
    .kpi-emerald::before { background: linear-gradient(90deg, #2546A8, #3157C7); }
    .metric-val-emerald { font-size: 2.3rem; font-weight: 800; color: #3157C7; line-height: 1.1; }
    .badge-bg-emerald { background: #EEF4FF; color: #2546A8; border: 1px solid #4F8EF7; }

    /* 🟧 Active JDA / QtQ % (Indigo Blue) */
    .kpi-orange {
        background: #FFFFFF !important;
        border-color: #D9E1EC;
    }
    .kpi-orange::before { background: linear-gradient(90deg, #3157C7, #4F8EF7); }
    .metric-val-orange { font-size: 2.3rem; font-weight: 800; color: #2546A8; line-height: 1.1; }
    .badge-bg-orange { background: #EEF4FF; color: #2546A8; border: 1px solid #4F8EF7; }

    /* 🟦 Active ME / YoY % (Royal Blue) */
    .kpi-blue {
        background: #FFFFFF !important;
        border-color: #D9E1EC;
    }
    .kpi-blue::before { background: linear-gradient(90deg, #2546A8, #3157C7); }
    .metric-val-blue { font-size: 2.3rem; font-weight: 800; color: #3157C7; line-height: 1.1; }
    .badge-bg-blue { background: #EEF4FF; color: #2546A8; border: 1px solid #4F8EF7; }

    /* 🔴 Active TME / Exited (Crimson Red Accent) */
    .kpi-rose {
        background: #FFFFFF !important;
        border-color: #D9E1EC;
    }
    ...
    .kpi-cyan::before { background: linear-gradient(90deg, #3157C7, #4F8EF7); }
    .metric-val-cyan { font-size: 2.3rem; font-weight: 800; color: #2546A8; line-height: 1.1; }
    .badge-bg-cyan { background: #EEF4FF; color: #2546A8; border: 1px solid #4F8EF7; }
    ...
    /* Executive Tables Card Container (#FFFFFF Table Background) */
    .custom-table-card {
        background: #FFFFFF !important;
    ...
    .custom-table th {
        background: linear-gradient(135deg, #2546A8 0%, #3157C7 100%) !important;
    ...
    .custom-table tr:nth-child(odd) td {
        background-color: #FFFFFF !important; /* Normal row */
    }
    .custom-table tr:nth-child(even) td {
        background-color: #F4F7FB !important; /* Alternate row */
    }
    .custom-table tr:hover td {
        background-color: #EAF1FF !important; /* Row hover */
        color: #172033 !important;
    }
    .custom-table tr.total-row td {
        background: linear-gradient(135deg, #3157C7 0%, #4F8EF7 100%) !important;
        color: #FFFFFF !important;
        font-weight: 800 !important;
        font-size: 1.05rem !important;
    }

    /* Export & Download Footer Banner (#FFFFFF) */
    .export-footer {
        background: #FFFFFF !important;
        backdrop-filter: blur(12px);
        border: 1.5px solid #D9E1EC;
        border-top: 4px solid #3157C7;
        border-radius: 20px;
    ...
    /* Executive Parent Tabs Track Container (Clean Transparent Track - No Outer White Bar) */
    .stTabs [role="tablist"],
    ...
    /* ALL Parent Tab Buttons (Both Active & Inactive) - Clean 1.35rem Card Scale */
    .stTabs button,
    ...
    /* ALL NESTED TEXT ELEMENTS IN PARENT TABS (FORCE 1.35rem FONT SIZE & BOLD 850 ACROSS ALL STATES) */
    .stTabs button *,
    .stTabs button p,
    .stTabs button span,
    .stTabs button div,
    ...
    div[data-testid="stTabs"] button:hover * {
        background: #F0F5FF !important;
        background-color: #F0F5FF !important;
        color: #3157C7 !important;
        border-color: #4F8EF7 !important;
    }

    /* ACTIVE PARENT TAB BUTTON - ROYAL SLATE & INDIGO BLUE PILL CARD (#3157C7 / #2546A8) */
    ...
    div[data-testid="stTabs"] [data-testid="stTab"][aria-selected="true"] {
        background: linear-gradient(135deg, #3157C7 0%, #2546A8 100%) !important;
        background-color: #3157C7 !important;
        border: 1.5px solid #3157C7 !important;
        border-radius: 14px !important;
        box-shadow: 0 6px 20px rgba(49, 87, 199, 0.4), 0 0 16px rgba(79, 142, 247, 0.3) !important;
    }
    ...
    /* Child Filter Segmented Control Bar - Subordinate Compact Control Bar */
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] {
        ...
        border: 1.5px solid #2546A8 !important;
        margin-top: 8px !important;
        margin-bottom: 1.5rem !important;
        width: fit-content !important;
        box-shadow: 0 3px 10px rgba(49, 87, 199, 0.25), 0 1px 4px rgba(23, 32, 51, 0.08) !important;
    }

    /* Unselected Child Filter Options (Subordinate 0.85rem Compact Scale) */
    ...
    /* ACTIVE SELECTED CHILD FILTER OPTION (WHITE SUB-PILL CARD) */
    .stTabs div[data-testid="stRadio"] div[role="radiogroup"] label:has(input:checked) {
        ...
        box-shadow: 0 2px 8px rgba(23, 32, 51, 0.14) !important;
        transform: none !important;
    }
...
<script>
    (function() {
        try {
            ...
            var styleId = 'custom-smooth-scroll-style-p5';
...
# 2. 👤 Employee Scope Category Dropdown (White Highlight Card)
with filter_c2:
...
# 3. 🏢 Branch Scope Category Dropdown (White Highlight Card)
with filter_c3:
...
# 4. 👥 Team Scope Category Dropdown (White Highlight Card)
with filter_c4:
...
def render_download_section(section_id="default"):
    ...
    <div class="export-footer">
        ...
        <div style="font-size: 0.92rem; color: #2546A8; margin-bottom: 0.9rem; font-weight:600;">
    ...
    if df_filtered.empty:
        ...
        if s_mode == "📊 Pan India / Branch Summary":
            ...
            for _, r in br_hc_df.iterrows():
                html_b.append('<tr>')
                html_b.append(f'<td style="font-weight:700; text-align:left; color:#2546A8;">{r["branch"]}</td>')
                ...
                html_b.append(f'<td style="color:#3157C7; font-weight:800;">{r["New_Joiners_Count"]}</td>')
                html_b.append('</tr>')
            ...
            for _, r in team_hc_df.iterrows():
                html_t.append('<tr>')
                html_t.append(f'<td style="font-weight:700; text-align:left; color:#2546A8;">{r["team_type"]}</td>')
                ...
                html_t.append(f'<td style="color:#3157C7; font-weight:800;">{r["New_Joiners_Count"]}</td>')
                html_t.append('</tr>')
    ...
    if df_filtered.empty:
        ...
        if v_mode == "⏳ Overall Tenure Buckets":
            ...
            for _, r in tb_df.iterrows():
                html_tb.append('<tr>')
                html_tb.append(f'<td style="font-weight:700; text-align:left; color:#2546A8;">{r["tenure_bucket"]}</td>')
                html_tb.append(f'<td style="color:#059669; font-weight:800;">{int(r["Active_Count"])}</td>')
                html_tb.append(f'<td style="color:#DC2626; font-weight:700;">{int(r["Exited_Count"])}</td>')
                html_tb.append(f'<td style="color:#3157C7; font-weight:800;">{r["Active_Pct"]}</td>')
                html_tb.append('</tr>')
            ...
            for _, r in b_pivot.iterrows():
                html_bp.append('<tr>')
                html_bp.append(f'<td style="font-weight:700; text-align:left; color:#2546A8;">{r["branch"]}</td>')
                for bo in bucket_order:
                    html_bp.append(f'<td>{r[bo]}</td>')
                html_bp.append(f'<td style="font-weight:800; color:#3157C7;">{r["Headcount"]}</td>')
                html_bp.append('</tr>')
            ...
            for _, r in t_pivot.iterrows():
                html_tp.append('<tr>')
                html_tp.append(f'<td style="font-weight:700; text-align:left; color:#2546A8;">{r["team_type"]}</td>')
                for bo in bucket_order:
                    html_tp.append(f'<td>{r[bo]}</td>')
                html_tp.append(f'<td style="font-weight:800; color:#3157C7;">{r["Headcount"]}</td>')
                html_tp.append('</tr>')
    ...
    if d_mode == "🔍 Searchable Employee List":
        ...
        if display_df.empty:
            ...
            for _, r in display_df.head(100).iterrows():
                ...
                html5.append('<tr>')
                html5.append(f'<td style="font-weight:700; color:#3157C7;">{r["emp_code"]}</td>')
                html5.append(f'<td style="text-align:left; font-weight:700; color:#172033;">{r["emp_name"]}</td>')
                html5.append(f'<td style="color:#2546A8;">{r["branch"]}</td>')
                html5.append(f'<td><b style="color:#059669;">{r["emp_type"]}</b></td>')
                html5.append(f'<td>{r["team_type"]}</td>')
                html5.append(f'<td>{r["doj"]}</td>')
                html5.append(f'<td>{dol_str}</td>')
                html5.append(f'<td style="font-weight:800; color:#059669;">{r["tenure_months"]} M</td>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_b_df.iterrows():
                ...
                html3.append(f'<td style="text-align:left; font-weight:700; color:#2546A8;">{r["Branch / City"]}</td>')
                html3.append(f'<td style="color:#059669; font-weight:800;">{r["Headcount"]}</td>')
                html3.append(f'<td style="color:#DC2626; font-weight:700;">{r["Exited in Period"]}</td>')
                html3.append(f'<td style="font-weight:800; color:#3157C7;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if df_filtered.empty:
            ...
            for _, r in full_t_df.iterrows():
                ...
                html4.append(f'<td style="text-align:left; font-weight:700; color:#2546A8;">{r["Team Type"]}</td>')
                html4.append(f'<td style="color:#059669; font-weight:800;">{r["Headcount"]}</td>')
                html4.append(f'<td style="color:#DC2626; font-weight:700;">{r["Exited in Period"]}</td>')
                html4.append(f'<td style="font-weight:800; color:#3157C7;">{r["Exact Avg Tenure (Months)"]} M</td>')
        ...
        if exited_df.empty:
            ...
            for _, r in exited_df.iterrows():
                ...
                html_ex.append(f'<td style="font-weight:800; color:#3157C7;">{r["tenure_months"]} M</td>')
